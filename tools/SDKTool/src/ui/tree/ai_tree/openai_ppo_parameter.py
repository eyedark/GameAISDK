...
class OpenAIPPOParameter(object):
    ...
    def init(self):
        if not self.__parameter:
            return self._ai_mgr.load_learning_config(AIAlgorithmType.OPENAI_PPO)
        return True
    ...
    def __get_params(self):
        if self._ai_parameter_inst is None:
            self._ai_parameter_inst = self._ai_mgr.get_ai_parameter(AIAlgorithmType.OPENAI_PPO)
