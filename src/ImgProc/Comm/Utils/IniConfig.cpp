...
int CIniConfig::getPrivateInt(const char *pszSectionName, const char *pszKeyName,
...
bool CIniConfig::getPrivateBool(const char *pszSectionName, const char *pszKeyName,
    const int nDefaultInt) {
    bool bRet = true;
...
int PostProcess(const char *pszDefaultStr, char *pszRetValue, const int nRetLen,
    char *pstrtmp, int ntmp) {
    if (*pszRetValue != '\0') {
        *pstrtmp = '\0';
...
int CIniConfig::ProcessOperateTwo(const char *pszDefaultStr, char *pszRetValue, const int nRetLen,
    char *pstrtmp, int ntmp, char  *pszOneLine, const int nBufLen, bool bApp,
...
// 处理App，Key都不为空的情况
int CIniConfig::ProcessOperateFour(const char *pszSectionName, const char *pszKeyName,
    const char *pszDefaultStr, char *pszRetValue, const int nRetLen, char *pstrtmp,
    int ntmp, char  *pszOneLine, const int nBufLen) {
    char *pszKey = new char[nBufLen];
    char *pszString = new char[nBufLen];
...
int CIniConfig::getPrivateProfileString(const char *pszSectionName, const char *pszKeyName,
    ...
    char szOneLine[LINE_BUFFER_LEN + 1], szKey[LINE_BUFFER_LEN + 1],
        szString[LINE_BUFFER_LEN + 1];
    ...
    if (operate == 1) {
        // 如果返回字符串长度小于2,全部返回空字符
        return ProcessOperateOne(pszDefaultStr, pszRetValue, nRetLen, pstrtmp, ntmp,
            szOneLine, LINE_BUFFER_LEN);
    }
