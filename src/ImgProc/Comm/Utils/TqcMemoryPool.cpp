...
PMEMORYPOOL CreateMemoryPool(void* pBuf, size_t sBufSize) {
    ...
    // 计算需要多少memory map单元格
    mem_pool->mem_map_pool_count = (sBufSize - mem_pool_struct_size + MINUNITSIZE - 1)
        / MINUNITSIZE;
    mem_pool->mem_map_unit_count = (sBufSize - mem_pool_struct_size + MINUNITSIZE - 1)
        / MINUNITSIZE;
    mem_pool->pmem_map = reinterpret_cast<memory_block*>(reinterpret_cast<char*>(pBuf)
        + mem_pool_struct_size);
    mem_pool->pfree_mem_chunk_pool
        = reinterpret_cast<memory_chunk*>(reinterpret_cast<char*>(pBuf)
        + mem_pool_struct_size + sizeof(memory_block) * mem_pool->mem_map_unit_count);

    mem_pool->memory = reinterpret_cast<char*>(pBuf) + mem_pool_struct_size
        + sizeof(memory_block) * mem_pool->mem_map_unit_count
        + sizeof(memory_chunk) * mem_pool->mem_map_pool_count;
    mem_pool->size = sBufSize - mem_pool_struct_size
        - sizeof(memory_block) * mem_pool->mem_map_unit_count
        - sizeof(memory_chunk) * mem_pool->mem_map_pool_count;
    ...
    // 链表化
    mem_pool->pfree_mem_chunk_pool = CreateList(mem_pool->pfree_mem_chunk_pool,
...
void* GetMemory(size_t sMemorySize, PMEMORYPOOL pMem) {
    ...
    if (max_chunk->chunk_size * MINUNITSIZE == sMemorySize) {
        ...
        pMem->pmem_map[current_index + current_block->count].count
            = copy.count - current_block->count;
    ...
    if (current_index == 0) {
        if (current_block->count < pMem->mem_block_count) {
            ...
            if (next_block->pmem_chunk != nullptr) {
                ...
                pMem->pmem_map[current_index + current_block->count + next_block->count - 1].start
                    = current_index;
                current_block->count += next_block->count;
