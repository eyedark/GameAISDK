...
int CDeformObjReg::Initialize(IRegParam *pParam) {
    ...
    for (int i = 0; i < static_cast<int>(m_oVecParams.size()); i++) {
        ...
        if (1 != nState) {
            LOGE("task ID %d: CDeformObjReg -- CYOLOAPI fill param failed, please check",
                m_nTaskID);
            return nState;
        }
        ...
        if (1 != nState) {
            oYOLOAPI.Release();
            LOGE("task ID %d: CDeformObjReg -- CYOLOAPI initialization failed, please check",
                m_nTaskID);
            return nState;
        }
...
int CDeformObjReg::Predict(const tagRegData &stData, IRegResult *pResult) {
    ...
    if (stData.nFrameIdx < 0) {
        LOGE("task ID %d: CDeformObjReg -- frame index %d is invalid, please check",
    ...
    if (NULL == pR) {
        LOGE("task ID %d: CDeformObjReg -- CDeformObjRegResult pointer is NULL, please check",
            m_nTaskID);
        return -1;
    }
    ...
    for (int i = 0; i < static_cast<int>(m_oVecYOLOAPIs.size()); i++) {
        ...
        if (oResult.m_oVecBBoxes.empty()) {
            ...
            szResults[i].nBBoxNum
                = MIN(static_cast<int>(oResult.m_oVecBBoxes.size()), MAX_BBOX_SIZE);
...
int CDeformObjReg::FillYOLOAPIParam(const tagDeformObjRegElement &stElement,
    CYOLOAPIParam &oParam) {
