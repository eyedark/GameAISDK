...
bool CTaskManager::Initialize(CThreadPoolMgr *pThreadPoolMgr, bool bSendSDKTool,
    bool bMultiResolution, char* pszSysCfgPath) {
    ...
    LOGI("gamereg platform cfg:%s", szPath);
    int        nRst = oIniConfig.loadFile(szPath);
    if (nRst != 0) {
        char szPath2[256] = { 0 };
        sprintf(szPath2, "%s%s", pszSysCfgPath, GAMEREG_PLATFORM_PRE_CFG);
        LOGI("gamereg platform cfg2:%s", szPath2);
        nRst = oIniConfig.loadFile(szPath2);
        if (nRst != 0) {
            LOGE("load file %s and load file %s failed", szPath, szPath2);
            return false;
        }
    }
    ...
    if (m_nOutTimeMS != INT_MAX) {
        if (m_nOutTimeMS >= nTimeOutMS) {
            *pFrameResult = m_mpFrameResult[nNeedFrameSeq];
            LOGD("expect size: %d, size: %d", pFrameResult->nResNum,
                static_cast<int>(pFrameResult->mapTaskResult.size()));
            m_nOutTimeMS = INT_MAX;
            LOGW("out of time");
            return true;
        } else {
...
//    LOGI("Task Result Queue size is %d", g_oTaskResultQueue.GetLength());
    ...
    if (stTmpFrameResult.mapTaskResult.size() == stTmpFrameResult.nResNum) {
        ...
        LOGD("expect size: %d, size: %d", stTmpFrameResult.nResNum,
            static_cast<int>(stTmpFrameResult.mapTaskResult.size()));
...
int CTaskManager::Update(const std::vector<CTaskMessage> &oVecTaskMsg,
    const tagSrcImgInfo &stSrcImageInfo, std::vector<tagTask> *pVecRst,
    ...
    *peSendTaskReportToMC = SEND_NO;
    ...
    if (!stSrcImageInfo.oSrcImage.empty() && m_ResFrameSeqQueue.size() < g_nMaxResultSize) {
        if (!m_oMapTaskCtx.empty()) {
            ...
            stRuntimeVar.oSrcImage = stSrcImageInfo.oSrcImage;
...
ESendTaskReportToMC CTaskManager::UpdateOneMsg(CTaskMessage *pMsg) {
    ...
    LOGI("Get Msg From Agent, Command ID:%d", uCmdID);
...
bool CTaskManager::ProcessGroupMsg(tagCmdMsg *pCmdMsg) {
    ...
    for (iterParam = pMsg->mapTaskParams.begin(); iterParam != pMsg->mapTaskParams.end();
        iterParam++) {
...
void TransSGBloodCoor(IRegResult *pRegResult, const float fScale) {
    ...
    CShootGameBloodRegResult *pShootGameBloodRegResult
    ...
    int                        nSize;
    tagShootGameBloodRegResult *pstShootGameBloodRegResult
        = pShootGameBloodRegResult->GetResult(&nSize);
...
void TransSGHurt(IRegResult *pRegResult, const float fScale) {
    ...
    CShootGameHurtRegResult *pShootGameHurtRegResult
    ...
    int                       nSize;
    tagShootGameHurtRegResult *pstShootGameHurtRegResult
        = pShootGameHurtRegResult->GetResult(&nSize);
...
void ShowDeforObjScale(cv::Mat *pTmpImage, IRegParam  *pRegParam, const int nID,
    const float fScale) {
    // 检查传入参数的合法性
    if (NULL == pTmpImage || NULL == pRegParam) {
        LOGE("input pointer is NULL, please check");
...
void ShowPixRegScale(cv::Mat *pTmpImage, IRegParam  *pRegParam, const int nID,
    const float fScale) {
    // 检查传入参数的合法性
    if (NULL == pTmpImage || NULL == pRegParam) {
        LOGE("input pointer is NULL, please check");
...
void ShowStuckRegScale(cv::Mat *pTmpImage, IRegParam  *pRegParam, const int nID,
    const float fScale) {
    // 检查传入参数的合法性
    if (NULL == pTmpImage || NULL == pRegParam) {
        LOGE("input pointer is NULL, please check");
...
void ShowNumberRegScale(cv::Mat *pTmpImage, IRegParam  *pRegParam, const int nID,
    const float fScale) {
    // 检查传入参数的合法性
    if (NULL == pTmpImage || NULL == pRegParam) {
        LOGE("input pointer is NULL, please check");
...
void ShowFixBloodRegScale(cv::Mat *pTmpImage, IRegParam  *pRegParam, const int nID,
    const float fScale) {
    // 检查传入参数的合法性
    if (NULL == pTmpImage || NULL == pRegParam) {
        LOGE("input pointer is NULL, please check");
...
void ShowKGBloodReg(cv::Mat *pTmpImage, IRegParam  *pRegParam, const int nID, const float fScale) {
    ...
    CKingGloryBloodRegParam *pKingGloryBloodRegParam
        = dynamic_cast<CKingGloryBloodRegParam*>(pRegParam);
    ...
    for (size_t nIdx = 0; nIdx < pKingGloryBloodRegParam->m_oVecElements.size(); ++nIdx) {
        ...
        tagKingGloryBloodRegParam stKingGloryBloodRegElement
            = pKingGloryBloodRegParam->m_oVecElements[nIdx];
        cv::Rect                  ROI;
...
void ShowMapRegScale(cv::Mat *pTmpImage, IRegParam  *pRegParam, const int nID,
    const float fScale) {
    // 检查传入参数的合法性
    if (NULL == pTmpImage || NULL == pRegParam) {
        LOGE("input pointer is NULL, please check");
...
void ShowMapDirRegScale(cv::Mat *pTmpImage, IRegParam  *pRegParam, const int nID,
    const float fScale) {
    // 检查传入参数的合法性
    if (NULL == pTmpImage || NULL == pRegParam) {
        LOGE("input pointer is NULL, please check");
...
void ShowSGBloodRegScale(cv::Mat *pTmpImage, IRegParam  *pRegParam, const int nID,
    const float fScale) {
    // 检查传入参数的合法性
    if (NULL == pTmpImage || NULL == pRegParam) {
        LOGE("input pointer is NULL, please check");
    ...
    CShootGameBloodRegParam *pShootGameBloodRegParam
        = dynamic_cast<CShootGameBloodRegParam*>(pRegParam);
    ...
    for (size_t nIdx = 0; nIdx < pShootGameBloodRegParam->m_oVecElements.size(); ++nIdx) {
        // adjust parameters apply to standard images
        tagShootGameBloodRegParam stShootGameBloodRegElement
            = pShootGameBloodRegParam->m_oVecElements[nIdx];
        cv::Rect                  ROI;
...
void ShowSGHurtRegScale(cv::Mat *pTmpImage, IRegParam  *pRegParam, const int nID,
    const float fScale) {
    // 检查传入参数的合法性
    if (NULL == pTmpImage || NULL == pRegParam) {
        LOGE("input pointer is NULL, please check");
    ...
    CShootGameHurtRegParam *pShootGameHurtRegParam
        = dynamic_cast<CShootGameHurtRegParam*>(pRegParam);
    ...
    for (size_t nIdx = 0; nIdx < pShootGameHurtRegParam->m_oVecElements.size(); ++nIdx) {
        // adjust parameters apply to standard images
        tagShootGameHurtRegParam stShootGameHurtRegElement
            = pShootGameHurtRegParam->m_oVecElements[nIdx];
        cv::Rect                 ROI;
    ...
    for (int idx = 0; idx < nSize; ++idx) {
        if (pszFixObjRes[idx].nState) {
            for (int nIdx = 0; nIdx < pszFixObjRes[idx].nBBoxNum; ++nIdx) {
                std::string       strTmp;
                std::stringstream sstream;
                sstream << static_cast<float>(static_cast<int>
                    (pszFixObjRes[idx].szBBoxes[nIdx].fScore * 100)) / 100;
                sstream >> strTmp;
    ...
    for (int idx = 0; idx < nSize; ++idx) {
        if (pszPixRes[idx].nState) {
            for (int nIdx = 0; nIdx < pszPixRes[idx].nPointNum; ++nIdx) {
                cv::circle(*pTmpImage, pszPixRes[idx].szPoints[nIdx], 1,
...
void ShowKingGloryBlood(cv::Mat *pTmpImage, IRegResult *pRegResult) {
    ...
    CKingGloryBloodRegResult *pKingGloryBloodRegRst
        = dynamic_cast<CKingGloryBloodRegResult*>(pRegResult);
    ...
    for (int idx = 0; idx < nSize; ++idx) {
        if (pszKingGloryBloodRes[idx].nState) {
            for (int nIdx = 0; nIdx < pszKingGloryBloodRes[idx].nBloodNum; ++nIdx) {
                std::string       strTmp;
                std::stringstream sstream;
                int nValue = static_cast<int>(
                    pszKingGloryBloodRes[idx].szBloods[nIdx].fPercent * 100);
    ...
    for (int idx = 0; idx < nSize; ++idx) {
        if (pszMapRes[idx].nState) {
            if (pszMapRes[idx].oMyLocPoint.x != -1) {
                cv::circle(*pTmpImage, pszMapRes[idx].oMyLocPoint, 2,
                    cv::Scalar(255, 255, 0), 1, 1, 0);
            }

            if (pszMapRes[idx].oViewLocPoint.x != -1) {
                cv::circle(*pTmpImage, pszMapRes[idx].oViewLocPoint, 2,
                    cv::Scalar(0, 255, 0), 1, 1, 0);
            }
...
void ShowMultiColorVar(cv::Mat *pTmpImage, IRegResult *pRegResult, const int nWidth,
    const int nHeight) {
    // 检查传入参数的合法性
    if (NULL == pTmpImage || NULL == pRegResult) {
        LOGE("input pointer is NULL, please check");
...
void ShowSGBloodRegResult(cv::Mat *pTmpImage, IRegResult *pRegResult) {
    ...
    CShootGameBloodRegResult *pShootGameBloodRegRst
        = dynamic_cast<CShootGameBloodRegResult*>(pRegResult);
    ...
    int                        nSize;
    tagShootGameBloodRegResult *pszShootGameBloodRes
        = pShootGameBloodRegRst->GetResult(&nSize);
    ...
    for (int idx = 0; idx < nSize; ++idx) {
        if (pszShootGameBloodRes[idx].nState) {
            ...
            cv::putText(*pTmpImage, strTmp.c_str(), cv::Point(pszShootGameBloodRes[idx].oROI.x - 5,
                ...
                cv::FONT_HERSHEY_DUPLEX, 0.7, cv::Scalar(0, 255, 0), 2);
            cv::rectangle(*pTmpImage, pszShootGameBloodRes[idx].stBlood.oRect,
                cv::Scalar(0, 255, 0), 2, 1, 0);
        }
    }
}

void ShowShootHurtResult(cv::Mat *pTmpImage, IRegResult *pRegResult, const int nSrcWdith,
    ...
    CShootGameHurtRegResult *pShootGameHurtRegRst
        = dynamic_cast<CShootGameHurtRegResult*>(pRegResult);
