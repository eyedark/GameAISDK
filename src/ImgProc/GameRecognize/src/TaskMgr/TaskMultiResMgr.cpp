...
bool CMutiResMgr::LoadReferElement(std::map<int, CTaskParam> *pmpTaskParam,
    const Json::Value &TaskList, const int nIdx, const int nTaskID) {
...
bool CMutiResMgr::LoadReferConfFile(std::map<int, CTaskParam> *pmpTaskParam,
    ...
    if (!root["alltask"].isArray() && !root["allTask"].isArray()) {
        LOGE("key of 'alltask or allTask' is needed and should be array in %s",
            strReferConfName.c_str());
        return false;
    }
    ...
    if (!root["alltask"][0]["groupID"].isInt()
        && !root["allTask"][0]["groupID"].isInt()) {
...
bool CMutiResMgr::LocationReferTask(CLocationRegParam *pLocationParam,
    ...
    if (pLocationParam->m_stParam.strAlgorithm == "Infer") {
        ...
        if (!VecRectParam(&pLocationParam->m_stParam.oVecInferLocations,
            oTask["inferLocations"])) {
            LOGE("get inferLocations failed");
            return false;
        }
    } else if (pLocationParam->m_stParam.strAlgorithm == "Detect") {
        LOGW("the algorithm is detect, please check");
    } else {
...
bool CMutiResMgr::BloodReferTask(CBloodLengthRegParam *pBloodLenParam, int nTaskID,
    ...
    if (!oTask["Conditions"].isNull()) {
        ...
        for (int nIdx = 0; nIdx < nSize; ++nIdx) {
            pBloodLenParam->m_oElement.oVecConditions
                .push_back(oTask["Conditions"][nIdx].asString());
        }
    }
    ...
    if (!ObjElements(&nVecElements, oTask["objElement"])
        && !ObjElements(&nVecElements, oTask["objElements"])) {
        LOGE("get objElement failed");
        return false;
    }
...
bool CMutiResMgr::VecTmplParam(std::vector<tagTmpl> *poVecTmpl, const Json::Value &oVecTmplValue) {
    ...
    for (int nIdx = 0; nIdx < nSize; ++nIdx) {
        ...
        oTmpl.strTmplPath = g_userCfgPath + oVecTmplValue[nIdx]["path"].asString();
...
void UpdateFixObjRegParam(IRegParam *pParam, cv::Rect *pRect, const int nRectSize,
    const float fScale, tagTaskElementArray &oVecObjElements) {
...
void UpdateStuckRegParam(IRegParam *pParam, cv::Rect *pRect, const int nRectSize,
    const float fScale, tagTaskElementArray &oVecObjElements) {
...
void UpdateDeformObjRegParm(IRegParam *pParam, cv::Rect *pRect, const int nRectSize,
    const float fScale, tagTaskElementArray &oVecObjElements) {
...
void UpdateFixBloodRegParam(IRegParam *pParam, cv::Rect *pRect, const int nRectSize,
    const float fScale, tagTaskElementArray &oVecObjElements) {
...
void UpdateKingGloryBlood(IRegParam *pParam, cv::Rect *pRect, const int nRectSize,
    const float fScale, tagTaskElementArray &oVecObjElements) {
    CKingGloryBloodRegParam *pKingGloryBloodRegParam
        = dynamic_cast<CKingGloryBloodRegParam*>(pParam);
...
void UpdateMapDirRegParam(IRegParam *pParam, cv::Rect *pRect, const int nRectSize,
    const float fScale, tagTaskElementArray &oVecObjElements) {
...
void UpdateSGBloodRegParam(IRegParam *pParam, cv::Rect *pRect, const int nRectSize,
    const float fScale, tagTaskElementArray &oVecObjElements) {
    CShootGameBloodRegParam *pShootGameBloodRegParam
        = dynamic_cast<CShootGameBloodRegParam*>(pParam);
...
bool CMutiResMgr::UpdateLocationParam(int nTaskID, std::map<int, TaskContext> *pmpTaskCtx,
    ...
    if (stLocationRegResult.nRectNum > 0) {
        ...
        UpdateTaskByReferTask(nTaskID, nObjTaskID, pmpTaskCtx);
    }

    return true;
}

void CMutiResMgr::UpdateTaskByReferTask(int nTaskID, int nObjTaskID,
    std::map<int, TaskContext> *pmpTaskCtx) {
...
bool CMutiResMgr::UpdateBloodRegParam(int nTaskID, std::map<int, TaskContext> *pmpTaskCtx,
    ...
    if (nState == 1) {
        ...
        if (!UpdateGameTaskParam(nTaskID, pTaskParam, &stBloodLengthRegResult.oROI,
            1, stBloodLengthRegResult.fScale)) {
            LOGE("Update game task param failed");
            return false;
        }
