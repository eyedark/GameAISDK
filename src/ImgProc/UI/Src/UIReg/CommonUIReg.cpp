...
bool CCommonUIReg::ChecSameImage(int nIndex, const tagFrameContext &stFrameCtx) {
    if (nIndex == -1 || m_nPreUID == -1) {
        return false;
    }

    if (0 == strcmp(m_oVecCfg[nIndex].strSampleFile, m_oVecCfg[m_nPreUID].strSampleFile)) {
...
int CCommonUIReg::Predict(const tagFrameContext &stFrameCtx, tagUIRegResult &stUIRegRst) {
    ...
    // 3) if  a) flag is true b) same frame count is less than threshold 3)
    // type is not UI_ACTION_DRAG_AND_CHECK
    // reset result index as -1

    if (nIndex != m_nPreUID) {
        LOGD("index is not equal to previous UID");
        bool bFlag = ChecSameImage(nIndex, stFrameCtx);
...
int CCommonUIReg::Predict(const cv::Mat &frame, const std::vector<tagCommonRegParam> &stParam,
    cv::Rect &DstRect) {
    m_nTmplMatchIndex = -1;
...
int CCommonUIReg::FindMatch(const std::vector<float> &fKPTVecMatch,
    const std::vector<bool> &bTmplVecMatch) {
    int   nIndex = -1;
    int   nMatchNum = 0;
    float fMaxScroe = 0;
