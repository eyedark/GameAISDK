# -*- coding: utf-8 -*-
# uncompyle6 version 3.7.5.dev0
# Python bytecode 3.5 (3350)
# Decompiled from: Python 3.7.10 (default, Apr 15 2021, 13:44:35) 
# [GCC 9.3.0]
# Embedded file name: ../../aisdk2/game_ai_sdk/tools/phone_aiclientapi/WrappedDeviceAPI/deviceAPI/mobileDevice/android/plugin/Platform_plugin/PlatformWeTest/demo/TcpSocketHandler.py
# Compiled at: 2020-12-29 09:26:44
# Size of source mod 2**32: 13030 bytes
import sys, time, socket, logging, struct, threading
_IS_PY2 = sys.version_info[0] < 3
if _IS_PY2:
    from Queue import Queue, Empty
else:
    from queue import Queue, Empty
from .pb.touch.TouchPkgPB_pb2 import *
from .pb.input.InputPkgPB_pb2 import *
from .pb.cloudscreen.CloudscreenPkgPB_pb2 import *
logger = logging.getLogger(__name__)
MSG_CLOSE_SOCKET = -1
RET_OK = 0
RET_ERR = -1
RET_ERR_SOCKET_EXCEPTION = -2

class TcpSocketHandler(threading.Thread):

    def __init__(self, ip, port, command_filter=None, is_screenmode=False, timeout=30.0):
        super(TcpSocketHandler, self).__init__(name=self.__class__.__name__)
        self.setDaemon(True)
        self.ip = ip
        self.port = port
        self.sock = None
        self.flag_cancel = False
        self.queue = Queue()
        self.is_screenmode = is_screenmode
        if is_screenmode:
            self.mutex = threading.Lock()
            self.last_packet = None
        else:
            self.res_queue = Queue()
        self.command_filter = command_filter
        self.event = threading.Event()
        self.heartEvent = threading.Event()
        self.reader = TcpSocketHandler.Reader(self)
        self.beater = TcpSocketHandler.HeartBeat(self)
        self.error = RET_OK
        self.has_error = True
        self.timeout = timeout
        self.packet_handler_maps = {}

    def __read_bytes(self, readlen):
        recv_len = 0
        _buffer = bytearray(readlen)
        buffer = memoryview(_buffer)
        while recv_len < readlen:
            ret = self.sock.recv_into(buffer[recv_len:], readlen - recv_len)
            if ret < 0:
                raise Exception('read error: readlen={}, get={}'.format(readlen, recv_len))
            recv_len += ret

        return buffer

    def read_packet(self):
        raise NotImplementedError

    def read_packet_bytes(self):
        if self.sock is None:
            raise Exception('sock is none')
        packet_len_bytes = self._TcpSocketHandler__read_bytes(4)
        packet_len, = struct.unpack('>I', packet_len_bytes)
        packet_bytes = self._TcpSocketHandler__read_bytes(packet_len)
        if len(packet_bytes) != packet_len:
            raise Exception('read error: readlen={}, get={}'.format(len(packet_bytes), packet_len))
        if _IS_PY2 and packet_bytes:
            packet_bytes = packet_bytes.tobytes()
        return packet_bytes

    def write_packet(self, cspkg):
        if self.sock is None:
            raise Exception('sock is none')
        packet_bytes = cspkg.SerializeToString()
        packet_len = len(packet_bytes)
        packet_len_bytes = struct.pack('>I', packet_len)
        buf = bytearray(packet_len_bytes)
        buf += bytearray(packet_bytes)
        sent_len = self.sock.send(buf)
        if sent_len != packet_len + 4:
            raise Exception('send packet error')

    def __close_socket(self):
        if self.sock:
            try:
                self.sock.close()
            except:
                logger.error('close socket exception')

    def is_heart_beat(self, pkg):
        return False

    def set_error(self):
        if self.is_screenmode:
            with self.mutex:
                self.error = RET_ERR_SOCKET_EXCEPTION

    def get_error(self):
        with self.mutex:
            return self.error

    def heart_beat(self):
        pass

    def close(self):
        logger.info('close socket, clear event')
        try:
            self._TcpSocketHandler__close_socket()
            self.error = RET_ERR_SOCKET_EXCEPTION
            self.has_error = True
            self.event.clear()
            with self.mutex:
                self.last_packet = None
        except:
            pass

    def quit(self):
        logger.info('quit reader thread...')
        self.flag_cancel = True
        self.event.set()
        self.reader.join(1)
        logger.info('quit reader thread...done')
        if self.beater and self.beater.is_alive():
            self.heartEvent.set()
            self.beater.join(1)
            logger.info('quit beater thread...done')
        logger.info('quit writer thread...')
        self.queue.put(MSG_CLOSE_SOCKET)
        self.join(1)
        logger.info('quit writer thread...done')

    class Reader(threading.Thread):

        def __init__(self, handler):
            super(TcpSocketHandler.Reader, self).__init__(name='Reader')
            self.setDaemon(True)
            self.handler = handler
            self.event = handler.event

        def run(self):
            while not self.handler.flag_cancel:
                logger.debug('---wait here')
                self.event.wait()
                self.event.clear()
                while not self.handler.flag_cancel:
                    try:
                        packet = self.handler.read_packet()
                        self.handler.error = RET_OK
                        self.handler.notify_heartthread()
                        if packet.header.command in self.handler.packet_handler_maps:
                            handler = self.handler.packet_handler_maps.get(packet.header.command)
                            ret = handler(packet)
                            if ret is False:
                                break
                        else:
                            if ret is True:
                                continue
                            if self.handler.is_heart_beat(packet):
                                logger.debug('>-- heart beat <--: {}'.format(packet))
                                continue
                            if self.handler.is_screenmode:
                                with self.handler.mutex:
                                    self.handler.last_packet = packet
                            else:
                                self.handler.res_queue.put(packet)
                    except Exception as e:
                        if e.errno != 10038:
                            logger.error('read packet exception', exc_info=True)
                        else:
                            logger.warning('error code(10038), socket may be closed')
                        logger.info('reader: put packet MSG_CLOSE_SOCKET')
                        self.handler.queue.put(MSG_CLOSE_SOCKET)
                        self.handler.set_error()
                        break

    class HeartBeat(threading.Thread):

        def __init__(self, handler):
            super(TcpSocketHandler.HeartBeat, self).__init__(name='HeartBeat')
            self.setDaemon(True)
            self.handler = handler
            self.event = handler.heartEvent

        def run(self):
            while not self.handler.flag_cancel:
                logger.debug('---wait here')
                while not self.handler.flag_cancel:
                    logger.debug('<-- heart beat -->')
                    self.handler.queue.put(self.handler.heart_beat())
                    time.sleep(5)

    def notify_heartthread(self):
        if not self.heartEvent.is_set():
            logger.info('set heartEvent')
            self.heartEvent.set()

    def run(self):
        self.reader.start()
        if self.heart_beat():
            self.beater.start()
        while not self.flag_cancel:
            sock = None
            try:
                sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                logger.info('connect {}:{}'.format(self.ip, self.port))
                try:
                    sock.connect((self.ip, self.port))
                    self.sock = sock
                    logger.info('set socket option')
                    self.sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
                    l_onoff = 1
                    l_linger = 0
                    self.sock.setsockopt(socket.SOL_SOCKET, socket.SO_LINGER, struct.pack('ii', l_onoff, l_linger))
                    self.sock.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)
                except:
                    logger.error('connect and setoption exception', exc_info=True)
                    self.close()
                    time.sleep(1)
                    continue

                while True:
                    try:
                        self.queue.get_nowait()
                    except Empty:
                        break

                if self.timeout:
                    self.sock.settimeout(self.timeout)
                if not self.event.is_set():
                    logger.info('set event')
                    self.event.set()
                if self.heart_beat():
                    self.queue.put(self.heart_beat())
                while not self.flag_cancel:
                    try:
                        packet = self.queue.get(timeout=1)
                    except Empty:
                        continue

                    try:
                        if packet is MSG_CLOSE_SOCKET:
                            break
                        logger.debug('write packet:{}'.format(packet))
                        self.write_packet(packet)
                    except:
                        logger.error('write packet exception', exc_info=True)
                        break

                self.close()
                logger.info('############### break here, not flag=' + str(not self.flag_cancel))
                time.sleep(1)
            finally:
                if sock:
                    sock.close()


class TouchSocketHandler(TcpSocketHandler):

    def __init__(self, ip, port, command_filter=None):
        super(TouchSocketHandler, self).__init__(ip, port, command_filter, is_screenmode=True)

    def read_packet(self):
        packet_bytes = self.read_packet_bytes()
        pkg = TouchPkg()
        pkg.ParseFromString(packet_bytes)
        return pkg

    def is_heart_beat(self, pkg):
        return pkg.header.command == TOUCH_HEARTBEAT_RES

    def heart_beat(self):
        pkg = TouchPkg()
        pkg.header.sequenceId = 9528
        pkg.header.timestamp = 0
        pkg.header.command = TOUCH_HEARTBEAT_REQ
        return pkg

    def get_last_packet(self):
        with self.mutex:
            packet = self.last_packet
            return (self.error, packet)


class InputSocketHandler(TcpSocketHandler):

    def __init__(self, ip, port, command_filter=None):
        super(InputSocketHandler, self).__init__(ip, port, command_filter, is_screenmode=True)

    def read_packet(self):
        packet_bytes = self.read_packet_bytes()
        pkg = InputPkg()
        pkg.ParseFromString(packet_bytes)
        return pkg

    def is_heart_beat(self, pkg):
        return pkg.header.command == INPUT_HEARTBEAT_RES

    def heart_beat(self):
        pkg = InputPkg()
        pkg.header.sequenceId = 9529
        pkg.header.timestamp = 0
        pkg.header.command = INPUT_HEARTBEAT_REQ
        return pkg

    def get_last_packet(self):
        with self.mutex:
            packet = self.last_packet
            return (self.error, packet)


class CloudscreenSocketHandler(TcpSocketHandler):

    def __init__(self, ip, port, command_filter=None):
        super(CloudscreenSocketHandler, self).__init__(ip, port, command_filter, is_screenmode=True)
        self.packet_handler_maps[SCREEN_CAPTURE_PUSH_MODE_STOP_NOTIFY] = self.handle_push_mode_stop_notify

    def read_packet(self):
        packet_bytes = self.read_packet_bytes()
        pkg = CloudscreenPkg()
        pkg.ParseFromString(packet_bytes)
        return pkg

    def is_heart_beat(self, pkg):
        return pkg.header.command == CLOUDSCREEN_HEARTBEAT_RES

    def heart_beat(self):
        pkg = CloudscreenPkg()
        pkg.header.sequenceId = 9527
        pkg.header.timestamp = 0
        pkg.header.command = CLOUDSCREEN_HEARTBEAT_REQ
        return pkg

    def get_last_packet(self):
        with self.mutex:
            packet = self.last_packet
            return (self.error, packet)

    def handle_push_mode_stop_notify(self, packet):
        """
        :param packet:
        :return: False, will break reader loop, True, will continue
        """
        logger.info('push mode stopped: {}'.format(packet))
        self.queue.put(MSG_CLOSE_SOCKET)
        self.set_error()
        return False