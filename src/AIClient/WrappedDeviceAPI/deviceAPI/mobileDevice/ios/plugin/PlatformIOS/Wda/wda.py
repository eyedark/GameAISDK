...
# [GCC 9.3.0]
# Embedded file name: ../../aisdk2/game_ai_sdk/tools/phone_aiclientapi/WrappedDeviceAPI/deviceAPI/mobileDevice/ios/plugin/PlatformIOS/Wda/wda.py
# Compiled at: 2020-12-29 09:26:39
# Size of source mod 2**32: 40404 bytes
from __future__ import print_function, unicode_literals
import base64, copy, functools, io, json, os, re, threading, time
from collections import defaultdict, namedtuple
import requests, retry, six
...
class WDAEmptyResponseError(WDAError):
    __doc__ = ' response body is empty '


class WDAElementNotFoundError(WDAError):
    __doc__ = ' element not found '


class WDAElementNotDisappearError(WDAError):
    __doc__ = ' element not disappera '
...
class Rect(object):
    ...
    def __str__(self):
        return 'Rect(x={x}, y={y}, width={w}, height={h})'.format(x=self.x, y=self.y, w=self.width, h=self.height)
...
class Client(object):
    ...
    def __init__(self, url=None):
        ...
        assert re.match('^https?://', url), 'Invalid URL: %r' % url
    ...
    def fast_screenshot_post(self, scale, quality):
        """
        fast screenshot
        :param scale: the image scale size you want to reduce, range: 0 < scale <= 1
        :param quality:the image quality you want to set,  range: 0 < quality <=1
        :return: raw jpeg data
        """
        data = {'scale': scale, 
         'quality': quality}
        b64data = self.http.post('/fastscreenshotpost', data=data).value
        raw_data = base64.b64decode(b64data)
        return raw_data

    def fast_screenshot(self, width, height):
        data = {'width': width, 
         'height': height}
        b64data = self.http.post('/fastscreenshot', data=data).value
        raw_data = base64.b64decode(b64data)
        return raw_data
...
class Session(object):
    ...
    def scale(self):
        ...
        """
        if self._Session__scale:
            return self._Session__scale
        v = max(self.screenshot().size) / max(self.window_size())
        self._Session__scale = round(v)
        return self._Session__scale
    ...
    def set_clipboard(self, content, content_type='plaintext'):
        """ set clipboard """
        self.http.post('/wda/setPasteboard', {'content': base64.b64encode(content.encode()).decode(), 
    ...
    def app_launch(self, bundle_id, arguments=[], environment={}, wait_for_quiescence=False):
        ...
        return self.http.post('/wda/apps/launch', {'bundleId': bundle_id, 
         'arguments': arguments, 
         'environment': environment, 
    ...
    def app_terminate(self, bundle_id):
        return self.http.post('/wda/apps/terminate', {'bundleId': bundle_id})
    ...
    def app_state(self, bundle_id):
        ...
        """
        return self.http.post('/wda/apps/state', {'bundleId': bundle_id})
    ...
    def swipe_hold(self, sx, sy, dx, dy, duration=1000):
        """
        swipe and hold for a moment
        :param sx: (int) start coordinate of x
        :param sy: (int) start coordinate of y
        :param dx: (int) the range of x to swipe to, eg:dx=50, swipe to (sx+50); dx=-50, swipe to (sx-50)
        :param dy: (int) the range of y to swipe to, eg:dy=50, swipe to (sy+50); dy=-50, swipe to (sy-50)
        :param duration: after swipe the time you hold(ms)
        :return:
        """
        data = {'actions': [
                     {'type': 'pointer', 
                      ...
                      'actions': [
                                  ...
                                  {'type': 'pointerMove', 'duration': duration, 'origin': 'pointer', 'x': dx, 'y': dy},
                                  {'type': 'pointerUp', 'button': 0}]}]}
        return self.http.post('/actions', data=data)
...
class Selector(object):
    ...
    def _wdasearch(self, using, value):
        ...
        """
        element_ids = []
        for v in self.http.post('/elements', {'using': using, 
         'value': value}).value:
    ...
    def _gen_class_chain(self):
        ...
        if self.visible is not None:
            qs.append('visible == true' if self.visible else 'false')
        if self.enabled is not None:
            qs.append('enabled == true' if self.enabled else 'false')
    ...
    def scroll(self, direction='visible', distance=1.0):
        ...
        if direction == 'visible':
            ...
            if direction in ('up', 'down', 'left', 'right'):
                self._wda_req('post', '/scroll', {'direction': direction, 
                 'distance': distance})
            else:
