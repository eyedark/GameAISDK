...
# Size of source mod 2**32: 11978 bytes
...
class ResourceApply(object):
    ...
    def init(self):
        ...
        if config.__contains__('communication') and config['communication'].__contains__('resource_apply_root_url'):
        ...
        if config.__contains__('apply_condition') and config['apply_condition'].__contains__('pre_apply'):
        ...
        if not ret:
            self.MAIN_THREAD_LOGGER.error('load_game_info failed!!')
            return (ret, error_str)
    ...
    def load_game_info(self, config):
        if os.environ.get('GAME_USE_ENV_VARIABLE'):
            ...
            if config.__contains__('resource') and config['resource'].__contains__('use_env_variable'):
    ...
    def check_game_info(self, user, game_id, scene_id, resolution, service, use_time, max_post_timeout, image_name):
        ...
        self.game_data = {'game_id': game_id, 'scene_id': scene_id, 'app_id': app_id, 'resolution': resolution, 
         'service': service, 'use_time': use_time}
        if image_name != '' and image_name is not None:
    ...
    def _load_parameter(self, cfg_file):
        if not os.path.exists(cfg_file):
            ...
            return (False, None, 'game.ini config file not exist:{}'.format(cfg_file))
        else:
            try:
    ...
    def load_pre_apply_result(self):
        if not os.path.exists(pre_apply_result_path):
            ...
            return (False, 'pre_apply_result_path not exist, path: {}'.format(pre_apply_result_path))
        else:
            try:
    ...
    def parse_testid_and_verify(self):
        ...
        try:
            if com_config.test_id == args.test_id and com_config.game_id == args.game_id and com_config.game_version == args.game_version:
