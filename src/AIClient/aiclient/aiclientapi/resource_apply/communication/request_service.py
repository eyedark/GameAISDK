...
class RequestService(object):
    ...
    def create_ai_service(self, data):
        url = "{}/{}".format(self.root_url, resource_apply_url)
        ret, result = self.http_post(url, data)
        if ret and result.get('error') == 0:
    ...
    def parse_ai_service(self, data):
        ...
        if data.get('code') == 200:
            return {'ip': data.get('tgw_ip'), 'port1': data.get('ports')[0], 'port2': data.get('ports')[1], 
             'key': data.get('source_server_id'), 
             'source_server_id': data.get('source_server_id'), 'task_id': data.get('task_id')}
