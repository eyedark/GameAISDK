...
# Size of source mod 2**32: 12468 bytes
...
class DeviceAPI(object):
    ...
    def init(self):
        ...
        try:
            ...
            self.MAIN_THREAD_LOGGER.error('device api instantiates failed')
            return (False, err)
        ...
        try:
            if self.device_type == DeviceType.Android.value:
                if self.platform == Platform.GAutomator.value:
                    ...
                    self.device_api.initialize(log_dir=self.log_dir, level=self.log_level, long_edge=self.long_edge, device_serial=self.device_serial, show_raw_screen=self.show_screen)
            else:
                if self.device_type == DeviceType.IOS.value:
                    self.device_api.initialize(log_dir=self.log_dir, level=self.log_level, long_edge=self.long_edge, device_serial=self.device_serial, show_raw_screen=self.show_screen)
                elif self.device_type == DeviceType.Windows.value:
                    self.device_api.initialize(log_dir=self.log_dir, level=self.log_level, hwnd=self.device_serial, window_size=self._windows_size, query_path=self._query_path)
            self.ready = True
        except:
    ...
    def finish(self):
        if self.device_api is not None:
            try:
                try:
                    if self.device_type == DeviceType.Android.value and self.platform == Platform.GAutomator.value:
