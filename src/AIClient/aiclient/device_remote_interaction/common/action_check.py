...
# [GCC 9.3.0]
# Embedded file name: ../../aisdk2/game_ai_sdk/tools/phone_aiclientapi/aiclient/device_remote_interaction/common/action_check.py
    ...
    def add_action(self, op_code, elapse_time=0):
        ...
        if self.action_sum_num % REMCORD_INTERVAL_NUM == 0:
            ...
            for op_code in self.action_num_dict.keys():
                self.MONITOR_LOGGER.info('op_code:{a}, action num:{b}, sum_consume_time:{c}, avg_consume_time:{d}'.format(a=op_code, b=self.action_num_dict[op_code], c=self.action_time_consume_dict[op_code], d=self.action_time_consume_dict[op_code] / self.action_num_dict[op_code]))
