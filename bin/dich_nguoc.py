...
class uncompyle6:
    ...
    def normalize_path(self, paths):
        ...
        for i in paths:
            ...
            base = self.swap_extension(self,i.replace(prefix, ""))
    ...
    def uncompyle(self, locations, parent):
        ...
        for i in locations:
            ...
            subprocess.call(["uncompyle6", "--encoding", "utf-8",  "-o", uncompyle_to, uncompyle_source])
...
if __name__ == "__main__":
    ...
    uncompyler.check_uncompyle(uncompyler)

    for root, dirs, files in os.walk(path):

        for result in uncompyler.search_pyc(uncompyler,root):

            matches.append(result)

    normalized = uncompyler.normalize_path(matches)

    uncompyler.setup_directories(uncompyler,normalized, cwd)

    uncompyler.uncompyle(uncompyler,normalized, cwd)
