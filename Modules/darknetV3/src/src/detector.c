...
#endif
        fprintf(stderr, "debug train_detector\n");
...
void validate_detector(char *datacfg, char *cfgfile, char *weightfile, char *outfile)
{
    ...
    fprintf(stderr, "debug validate_detector\n");
...
{
    fprintf(stderr, "debug validate_detector_recall\n");
    network net = parse_network_cfg_custom(cfgfile, 1, 1);    // set batch=1
    if (weightfile) {
        load_weights(&net, weightfile);
    }
...
float validate_detector_map(char *datacfg, char *cfgfile, char *weightfile, float thresh_calc_avg_iou, const float iou_thresh, const int map_points, int letter_box, network *existing_net)
{
    ...
    if (existing_net) {
        ...
        fprintf(stderr, "debug validate_detector_map\n");
    ...
    float hier_thresh, int dont_show, int ext_output, int save_labels, char *outfile, int letter_box, int benchmark_layers)
{
    ...
    fprintf(stderr, "debug test_detector\n");
    network net = parse_network_cfg_custom(cfgfile, 1, 1); // set batch=1
    if (weightfile) {
        load_weights(&net, weightfile);
    }
