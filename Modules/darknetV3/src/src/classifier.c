...
#endif
        fprintf(stderr, "debug train_classifier\n");
...
{
    int i = 0;
    fprintf(stderr, "debug validate_classifier_crop\n");
...
{
    int i, j;
    fprintf(stderr, "debug validate_classifier_10\n");
    network net = parse_network_cfg(filename);
...
{
    int i, j;
    fprintf(stderr, "debug validate_classifier_full\n");
    network net = parse_network_cfg(filename);
    set_batch_network(&net, 1);
    if(weightfile){
        load_weights(&net, weightfile);
...
float validate_classifier_single(char *datacfg, char *filename, char *weightfile, network *existing_net, int topk_custom)
{
    ...
    if (existing_net) {
        ...
        fprintf(stderr, "debug validate_classifier_single\n");
...
{
    int i, j;
    fprintf(stderr, "debug validate_classifier_multi\n");
    network net = parse_network_cfg(filename);
    set_batch_network(&net, 1);
    if(weightfile){
        load_weights(&net, weightfile);
...
{
    fprintf(stderr, "debug try_classifier\n");
...
{
    fprintf(stderr, "debug predict_classifier\n");
    network net = parse_network_cfg_custom(cfgfile, 1, 0);
    if(weightfile){
        load_weights(&net, weightfile);
    }
...
{
    int i;
    fprintf(stderr, "debug label_classifier\n");
    network net = parse_network_cfg(filename);
    set_batch_network(&net, 1);
    if(weightfile){
        load_weights(&net, weightfile);
...
{
    int curr = 0;
    fprintf(stderr, "debug test_classifier\n");
    network net = parse_network_cfg(cfgfile);
    if(weightfile){
        load_weights(&net, weightfile);
    }
